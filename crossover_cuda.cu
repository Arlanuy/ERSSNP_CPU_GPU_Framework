...
__host__ create_rule(Rule rule, int* source, int* sink, int* prod, int* con, int* delay, int* size_rssnp) {
    rule.source = source;
    rule.sink = sink;
    rule.prod = prod;
    rule.con = con;
...
void main() {
    ...
    funcCheck(cudaMalloc((void **)&d_rules, sizeof(Rules)*max_rules);
    funcCheck(cudaMalloc((void **)&d_rssnp_indexes_pair1, sizeof(int)*max_rssnp));
    funcCheck(cudaMalloc((void  **)&d_rssnp_indexes_pair2, sizeof(int)*max_rssnp));
    ...
    funcCheck(cudaMalloc((void **)&list_poss_rssnp, max_rssnp * sizeof(int));
    funcCheck(cudaMalloc((void **)&list_poss_rules, max_rules * sizeof(int)));
    funcCheck(cudaMalloc((void  **)&size_rssnp, max_rssnp * sizeof(int)));
