import yaml, os
yaml_averager_out = open(os.getcwd()+ "\\yamlaverageroutreal.txt", "w+")
			...
			if gpu_format == True:
				if run >= 5:
					...
					for gen in data['runs'][run]['generations']:
						...
						total_len_rules += sum(rules_array)/len(rules_array)
						total_fitness += sum(fitness_array)/len(fitness_array)
