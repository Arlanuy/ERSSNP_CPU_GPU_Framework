...
def timer_write(ga_name, start, finish):
    timer_out_cpu = open(os.getcwd()+ "\\timer_directory\\gpuaddadversarial11outreal.txt", "a+")
    timer_out_cpu.write(ga_name + " GPU time is " + str(finish - start) + "\n")
    timer_out_cpu.close()

def timer_write2(ga_name, start, finish):
    timer_out_cpu = open(os.getcwd()+ "\\timer_directory\\2gpuaddadversarial11outreal.txt", "a+")
