...
__global__ void lc_substring(int j, int *X,int *Y,int *res, int *LCSuff,int row_width){
   int i = blockIdx.x * blockDim.x + threadIdx.x;
   if (i < row_width){
            if (i == 0){
                LCSuff[i*row_width+j] = j;
                __syncthreads();
            }
            else if (j == 0){
                LCSuff[i*row_width+j] = i;
                __syncthreads();
            }
            else if (X[i-1] == Y[j-1]) {  
                LCSuff[i*row_width+j] = LCSuff[((i-1)*row_width)+j-1];
                __syncthreads();
                
            } 
            else{
            	
                LCSuff[i*row_width+j] = 1+min1(min1(LCSuff[((i-1)*row_width)+j],LCSuff[i*row_width+j-1]),LCSuff[((i-1)*row_width)+j-1]);
                __syncthreads();
            }
    } 
}
...
for i in range(b.size+1):
    LCS(numpy.int32(i),a_gpu,b_gpu,LCSuff_gpu,numpy.int32(a.size+1), block=(10,10,1),grid=(1,1,1))
