...
__global__ void lc_subsequence(int j, int *X,int *Y,int *res, int *LCSuff,int row_width,int col_width){
   ...
   if (i < row_width){
            ...
            if (i == 0 || j == 0){
                ...
                temp = max1(LCSuff[(i-1)*col_width+j],LCSuff[i*col_width+j-1]);
                __syncthreads();
                LCSuff[i*col_width+j] = temp;
...
for i in range(b.size+1):
    LCS(numpy.int32(i),a_gpu,b_gpu,res_gpu,LCSuff_gpu, numpy.int32(a.size+1),numpy.int32(b.size+1), block=(10,1,1),grid=(1,1,1))
